/**
 * Certificado intermediario necessario para falar com balanca.economia.gov.br.
 *
 * POR QUE ISTO EXISTE: o servidor da Receita/SERPRO envia APENAS o certificado
 * folha, sem os intermediarios. O Windows resolve sozinho (busca o intermediario
 * pelo campo AIA), mas o Node nao faz isso — o erro que aparece e
 * UNABLE_TO_VERIFY_LEAF_SIGNATURE.
 *
 * A saida preguicosa seria NODE_TLS_REJECT_UNAUTHORIZED=0, que desliga a
 * verificacao TLS do processo inteiro. Inaceitavel numa ferramenta que outras
 * pessoas instalam: abriria a porta a man-in-the-middle em qualquer conexao.
 * Em vez disso empacotamos o elo que falta e mantemos a verificacao LIGADA.
 *
 *   Emissor da folha : Sectigo Public Server Authentication CA OV R36
 *   Encadeia em      : Sectigo Public Server Authentication Root R46
 *                      (essa raiz JA esta no bundle do Node — so falta o meio)
 *   Valido ate       : 2036-03-21
 *   Origem           : http://crt.sectigo.com/SectigoPublicServerAuthenticationCAOVR36.crt
 *                      (URL declarada no proprio AIA do certificado servido)
 *
 * Para atualizar quando vencer, baixe o .crt do AIA e converta:
 *   openssl x509 -inform DER -in novo.crt -out novo.pem
 */
